...
class ARVCalculationService {
    ...
    calculateARV(comparables, subjectSqft, subjectBaths) {
        ...
        const subjectBathsNormalized = Number.isFinite(subjectBaths)
            ? Number(subjectBaths)
            : null;
        ...
        const needsBathAdjustments = subjectBathsNormalized != null
            && finalComparables.some(comp => Number.isFinite(comp.baths) && comp.baths > subjectBathsNormalized);
     ...
     */
    modeALargeSampleOutlierDetection(comparables) {
     ...
     */
    modeBSmallSampleOutlierDetection(comparables) {
     ...
     */
    applyCoreOutlierDetection(comparables, subjectSqft) {
        if (comparables.length < 3) {
            return comparables;
        }
    ...
    estimateSecondBathPremium(subjectSqft, comps) {
        ...
        const bucket = (c) => Math.abs(c.sqft - subjectSqft) <= subjectSqft * 0.10;
