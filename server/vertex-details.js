...
Provide specific facts with numbers. If any critical data is missing, clearly state "MISSING" for that field.`;
    ...
    if (!hasCriticalData) {
        ...
        if (!propertyDetails.sqft)
            missingFields.push('square footage');
        if (!propertyDetails.beds)
            missingFields.push('bedrooms');
        if (!propertyDetails.baths)
            missingFields.push('bathrooms');
        if (!propertyDetails.yearBuilt)
            missingFields.push('year built');
        try {
...
Focus only on finding: ${missingFields.join(', ')}. Provide exact numbers.`;
            ...
            if (!propertyDetails.sqft && countyData.sqft)
                propertyDetails.sqft = countyData.sqft;
            if (!propertyDetails.beds && countyData.beds)
                propertyDetails.beds = countyData.beds;
            if (!propertyDetails.baths && countyData.baths)
                propertyDetails.baths = countyData.baths;
            if (!propertyDetails.yearBuilt && countyData.yearBuilt)
    ...
    if (!finalValidation) {
        const stillMissing = [];
        if (!propertyDetails.sqft)
            stillMissing.push('square footage');
        if (!propertyDetails.beds)
            stillMissing.push('bedrooms');
        if (!propertyDetails.baths)
            stillMissing.push('bathrooms');
        if (!propertyDetails.yearBuilt)
            stillMissing.push('year built');
