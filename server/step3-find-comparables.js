...
address | sold_price | sold_date(YYYY-MM-DD) | beds | baths | sqft | year_built | source_url`;
            ...
            for (const list of batches) {
                list.forEach(c => { if (!aggregatedComps.has(c.address))
                    aggregatedComps.set(c.address, c); });
            }
...
Return exactly this JSON structure:
{
  ...
  "source": "website name or URL"
                ...
                if (soldDateStr && soldDateStr.toUpperCase() !== 'INVALID') {
                    ...
                    if (!isNaN(d.getTime()))
                        ...
                        if (m)
                            ...
                            if (m)
                                soldDate = new Date(parseInt(m[3], 10), parseInt(m[1], 10) - 1, parseInt(m[2], 10));
                        }
                    }
                }
                results[id] = {
    ...
    async parseVertexResponse(text, subjectLat, subjectLon, subjectDetails) {
        ...
        const cheapParse = (parts) => {
            ...
            if (!hasAddress || !hasPrice || !hasDate)
                return null; // Core fields required
            return {
                address,
                price,
                soldDate,
        ...
        for (const idx of validLineIndex) {
            ...
            if (!price || !Number.isFinite(price))
                price = parseIntSafe(priceStr);
            if (!soldDate)
                soldDate = parseDateSafe(dateStr);
            if (!beds || !Number.isFinite(beds))
                beds = parseIntSafe(bedsStr);
            if (!baths || !Number.isFinite(baths))
                baths = parseFloatSafe(bathsStr);
            if (!sqft || !Number.isFinite(sqft))
                sqft = parseIntSafe(sqftStr);
            if (!yearBuilt || !Number.isFinite(yearBuilt))
                yearBuilt = parseIntSafe(ybStr);
    ...
    prioritizeForEnrichment(comps) {
        const score = (c) => {
            ...
            if (Number.isFinite(c.price))
                s += 2;
            if (Number.isFinite(c.sqft))
                s += 2;
            if (Number.isFinite(c.beds))
                s += 1;
            if (Number.isFinite(c.baths))
                s += 1;
            if (c.yearBuilt != null)
                s += 1;
            if (c.soldDate)
    ...
    async geocodeAndFilterDistance(comps, subjectLat, subjectLon, idealMiles = 1.0, maxMiles = 2.0) {
        const out = [];
        ...
        await new Promise((resolve) => {
            const next = () => {
                if (index >= comps.length && active === 0)
                    return resolve();
    ...
    filterByPPSFVariance(comps) {
        ...
        if (valid.length < 3)
            return comps; // Not enough to compute outliers
        ...
        for (const comp of comps) {
            ...
            if (needsEnrich) {
                try {
                    ...
                    if (details) {
                        if (!Number.isFinite(updated.beds) && details.beds != null)
                            updated.beds = details.beds;
                        if (!Number.isFinite(updated.baths) && details.baths != null)
                            updated.baths = details.baths;
                        if (updated.yearBuilt == null && details.yearBuilt != null)
                            updated.yearBuilt = details.yearBuilt;
                        if (!Number.isFinite(updated.sqft) && details.sqft != null)
                            updated.sqft = details.sqft;
                    }
                }
                catch { }
            }
            // Require full set after enrichment
            const hasAll = (updated.address &&
            ...
            if (Number.isFinite(updated.price) && (updated.price < 50000 || updated.price > 5000000))
                continue;
            if (Number.isFinite(updated.sqft) && (updated.sqft < 500 || updated.sqft > 10000))
                continue;
            if (Number.isFinite(updated.beds) && (updated.beds < 1 || updated.beds > 10))
                continue;
            if (Number.isFinite(updated.baths) && (updated.baths < 1 || updated.baths > 10))
                continue;
            if (subjectDetails) {
                ...
                if (Number.isFinite(updated.baths)) {
                    ...
                    if (subjectDetails.baths > 2 && bathDiff > 1.5)
                        continue;
                }
                ...
                if (Number.isFinite(updated.yearBuilt)) {
                    ...
                    if (!this.isAdjacentAgeGroup(ageGroup, compAge))
                        continue;
                }
            }
    ...
    async calculateDistance(address, subjectLat, subjectLon, timeoutMs = 3000) {
        try {
            const geocoded = await this.geocodeWithTimeout(address, timeoutMs);
            if (!geocoded)
                throw new Error('geocode-timeout');
...
export { VertexComparableSearchService };
