...
exports.ComprehensiveCompSearchV3 = void 0;
const step3_find_comparables_js_1 = require("./step3-find-comparables.js");
const step4_arv_calculation_js_1 = require("./step4-arv-calculation.js");
const vertex_details_js_1 = require("./vertex-details.js");
const propertyDataNormalizer_js_1 = require("./utils/propertyDataNormalizer.js");
const smartDeduplicator_js_1 = require("./utils/smartDeduplicator.js");
const progressiveSearchStrategy_js_1 = require("./utils/progressiveSearchStrategy.js");
const distanceValidator_js_1 = require("./utils/distanceValidator.js");
class ComprehensiveCompSearchV3 {
    constructor() {
        this.compService = new step3_find_comparables_js_1.VertexComparableSearchService();
        this.arvService = new step4_arv_calculation_js_1.ARVCalculationService();
        this.normalizer = new propertyDataNormalizer_js_1.PropertyDataNormalizer();
        this.deduplicator = new smartDeduplicator_js_1.SmartDeduplicator();
        this.progressiveSearch = new progressiveSearchStrategy_js_1.ProgressiveSearchStrategy();
        this.distanceValidator = new distanceValidator_js_1.DistanceValidator();
    }
    async findComparables(address) {
        ...
        try {
            ...
            const subjectDetails = await (0, vertex_details_js_1.fetchPropertyDetailsViaVertex)(address);
            if (!subjectDetails) {
            ...
            if (subjectDetails.beds && subjectDetails.baths && subjectDetails.sqft && subjectDetails.yearBuilt) {
                ...
                for (searchLevel = 0; searchLevel < maxSearchLevels; searchLevel++) {
                    ...
                    const result = await this.compService.findComparables(address, undefined, // propertyType
                    maxResults, radius, timeWindow, subjectDetails);
            ...
            console.log(`\n📏 Step 5: Distance Validation`);
            const distanceResult = await this.distanceValidator.validateComparableDistances(address, deduplicatedComps, 2.5 // max miles
            );
            ...
            const qualifiedComps = distanceValidatedComps.filter((_, index) => consistencyScores.get(index.toString()) && consistencyScores.get(index.toString()) > 0.6);
            ...
            if (qualifiedComps.length >= 3 && subjectDetails.sqft) {
                ...
                if (subjectBaths < 2 && arvResult) {
                    ...
                    if (twoBathComps.length >= 3 && baselineComps.length >= 3) {
                        ...
                        bathroomAnalysis.recommendAction = this.generateRecommendation(arvResult.estimate, upgradeARV.arv, arvResult.confidence);
     ...
     */
    computeSubjectBathrooms(propertyDetails) {
    ...
    filterComparablesForBaseline(comps, subjectBaths) {
        ...
        return comps.filter(comp => {
            ...
            if (!Number.isFinite(compBaths))
                return false;
            // Only use comps with baths <= subject (conservative approach)
            if (compBaths > subjectBaths + epsilon)
                return false;
            // Additional quality filters
            if (!Number.isFinite(comp.price) || !Number.isFinite(comp.sqft))
                return false;
    ...
    filterComparablesForTwoBath(comps, subjectDetails) {
        return comps.filter(comp => {
            ...
            if (!Number.isFinite(compBaths) || compBaths < 2)
                return false;
    ...
    generateRecommendation(baselineARV, twoBathARV, confidence) {
        if (!twoBathARV)
            return 'sell_as_is'; // No upgrade scenario available
        const valueAdd = twoBathARV - baselineARV;
        const estimatedRenovationCost = 12000; // Typical bathroom addition cost
        const netGain = valueAdd - estimatedRenovationCost;
        const roi = (netGain / estimatedRenovationCost) * 100;
        // Decision logic
        if (confidence === 'high' && roi > 25)
            return 'renovate';
        if (confidence === 'medium' && roi > 40)
            return 'renovate';
        if (confidence === 'low' && roi > 60)
            return 'renovate';
        if (baselineARV > 200000)
            return 'hold'; // Good value, hold for appreciation
        return 'sell_as_is';
    }
    ...
    assessOverallQuality(compCount, consistencyScores) {
        ...
        if (compCount >= 8 && avgConsistency >= 0.8)
            return 'excellent';
        if (compCount >= 5 && avgConsistency >= 0.7)
            return 'good';
        if (compCount >= 3 && avgConsistency >= 0.6)
            return 'fair';
        return 'poor';
    }
}
exports.ComprehensiveCompSearchV3 = ComprehensiveCompSearchV3;
// Test function
