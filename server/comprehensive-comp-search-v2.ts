...
interface ComprehensiveSearchResultV2 {
  all_comps: any[];
  qualified_comps: any[];
...
export class ComprehensiveCompSearchV2 {
  ...
  async performOptimalSearchV2(
    address: string,
    subjectDetails?: { sqft: number; beds: number; baths: number; yearBuilt: number; subdivision?: string }
  ): Promise<ComprehensiveSearchResultV2> {
    ...
    const deduplicationResult = this.deduplicator.deduplicateProperties(
   ...
   */
  private analyzeRenovationStatus(
    properties: any[],
