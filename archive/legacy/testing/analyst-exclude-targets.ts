...
async function main() {
  ...
  // Exclusions derived from user targets (avoid mentioning any of these streets)
  const exclusions = [
    'Bailey Ct',
    'Stoneridge Way',
    'Ridgemont',
  ...
  const exclusionLine = exclusions.map(e => `- ${e}`).join('\n');
...
EXCLUSIONS:
Do not include any comps on streets or addresses containing these names (avoid them completely):
${exclusionLine}
...
Now, provide the comps.`;
  ...
  console.log('🧪 Running analyst prompt with exclusions (no target street names in results)...');
  const text = await vertexGenerateRaw({ token, location, projectId, model, prompt });
  console.log('\n--- Analyst Exclusions Output ---');
  console.log(text);
}

main().catch(err => { console.error('Analyst exclude-targets test failed:', err?.message || err); process.exit(1); });
