...
- Store credentials automatically for local development
- Provide access to Vertex AI, Cloud Logging, and other GCP services
...
**Solution**: Verify you're authenticated with Google Cloud:
```bash
...
**Solution**: Check your account has Vertex AI permissions:
```bash
...
**Note**: Authentication via `gcloud auth application-default login` is a one-time setup and persists across sessions.
